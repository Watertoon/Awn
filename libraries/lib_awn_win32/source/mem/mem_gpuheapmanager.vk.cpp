...
namespace awn::mem {
	...
	bool GpuHeapManager::AllocateContext(GpuRootHeapContext *out_heap_context, const char *name, size_t size, gfx::MemoryPropertyFlags memory_properties) {
		...
		out_heap_context->root_heap = mem::GpuExpHeap::TryCreate(name, out_heap_context, out_heap_context->base_address, size);
	...
	bool GpuHeapManager::Initialize(const GpuHeapManagerInfo *gpu_heap_mgr_info) {
		...
		for (u32 i = 0; i < gpu_heap_mgr_info->host_uncached_root_heap_count; ++i) {
			...
			this->AllocateContext(std::addressof(m_host_uncached_heap_context_array[i]), cpu_uncached_name_array[i], gpu_heap_mgr_info->host_uncached_size_array[i], gfx::MemoryPropertyFlags::CpuUncached);
		}
		for (u32 i = 0; i < gpu_heap_mgr_info->host_cached_root_heap_count; ++i) {

			/* Allocate host cached memory */
			this->AllocateContext(std::addressof(m_host_cached_heap_context_array[i]), cpu_cached_name_array[i], gpu_heap_mgr_info->host_cached_size_array[i], gfx::MemoryPropertyFlags::CpuCached);
		}
		for (u32 i = 0; i < gpu_heap_mgr_info->gpu_host_uncached_root_heap_count; ++i) {

			/* Allocate gpu host uncached memory */
			this->AllocateContext(std::addressof(m_gpu_host_uncached_heap_context_array[i]), cpu_uncached_gpu_cached_name_array[i], gpu_heap_mgr_info->gpu_host_uncached_size_array[i], gfx::MemoryPropertyFlags::GpuUncached | gfx::MemoryPropertyFlags::CpuUncached);
		}
